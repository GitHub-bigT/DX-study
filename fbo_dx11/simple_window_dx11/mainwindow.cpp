...
void MainWindow::createWindowWithWin32(WindowData &wd)
{
	...
	wd.hwnd = CreateWindowEx(WS_EX_APPWINDOW, wd.applicationName, wd.applicationName,
		WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_POPUP,//无边框,不能调节窗体大小
		//WS_POPUP  |  WS_THICKFRAME的话有点细小的边框,可以用鼠标调节窗体大小
		//WS_OVERLAPPEDWINDOW,
...
void MainWindow::startEngine()
{
	for (int i = 0; i < m_GpuCount; i++)
	{
		...
		ttd.wd.x = 0; ttd.wd.y = 0; ttd.wd.width = 1920; ttd.wd.height = 1080; ttd.wd.gpu = 0; ttd.wd.number = -1;
		createWindowWithWin32(ttd.wd);
		ttd.texThread = new TextureThread(i, ttd.wd, m_WindowVec);
