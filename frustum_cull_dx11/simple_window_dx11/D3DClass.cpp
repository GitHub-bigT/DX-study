...
/*
	...
	m_orthoMatrix = XMMatrixOrthographicOffCenterLH(-(float)screenWidth / 2.0f, (float)screenWidth / 2.0f, -(float)screenHeight / 2.0f, (float)screenHeight / 2.0f, 0.0f, screenDepth);
	...
	hr = m_device->CreateBlendState(&blendStateDescription, &m_alphaEnableBlendState);
	if (FAILED(hr))
	{
		return false;
	}
	...
	hr = m_device->CreateBlendState(&blendStateDescription, &m_alphaDisableBlendState);
	if (FAILED(hr))
	{
		return false;
	}
