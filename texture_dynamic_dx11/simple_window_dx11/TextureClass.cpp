...
bool TextureClass::init(ID3D11Device *device, ID3D11DeviceContext *deviceContext, char *filename)
{
	...
	hr = device->CreateTexture2D(&textureDesc, 0, &m_texture);
	if (FAILED(hr))
	{
		return false;
	}
...
bool TextureClass::updateTexture(ID3D11DeviceContext *deviceContext, char *filename)
{
	bool result;
	result = loadImage(filename);
	if (!result)
	...
	deviceContext->UpdateSubresource(m_texture, 0, &box, m_targaData, rowPitch, rowPitch * m_height);
/*
	D3D11_MAPPED_SUBRESOURCE mapped;
	deviceContext->Map(m_texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped);
	memcpy(mapped.pData, m_targaData, m_width * m_height * 4);
